  ...
  >
    ...
    <td class="px-2 py-2.5 text-right">
      <div class="flex items-center justify-end gap-1">
        ...
        <!-- Quiz Manager (chỉ hiện khi type=quiz) -->
        <button
          v-if="lesson.type === 'quiz'"
          @click="$emit('manage-quiz', lesson)"
          class="p-1 text-purple-400 hover:text-purple-600 hover:bg-purple-50 rounded-lg dark:hover:bg-purple-500/10 transition-colors"
...
const typeLabel = computed(
  () =>
    ({ video: 'Video', document: 'Tài liệu', quiz: 'Bài kiểm tra' })[props.lesson.type] ||
    props.lesson.type,
)
