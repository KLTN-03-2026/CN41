<template>
  ...
  >
      ...
      <div class="flex items-center gap-3">
        ...
        <div class="min-w-0">
          <p
            ...
            :class="
              isTrashed ? 'text-gray-500 dark:text-gray-400' : 'text-gray-800 dark:text-gray-200'
            "
          >
            {{ course.name }}
          </p>
    ...
    <td
      class="px-6 py-4"
      :class="isTrashed ? 'text-gray-500 dark:text-gray-500' : 'text-gray-600 dark:text-gray-400'"
    >
      {{ course.teacher?.name || '—' }}
    </td>
    <td class="px-6 py-4">
...
function levelLabel(level: string) {
  return (
    (
      { beginner: 'Cơ bản', intermediate: 'Trung cấp', advanced: 'Nâng cao' } as Record<
        string,
        string
      >
    )[level] || level
