...
async function loadQuiz() {
  try {
    const res = await quizService.teacherLessonQuizGet(props.lessonId)
...
async function loadChapterPdfs() {
  ...
  try {
    const res = await quizService.teacherLessonQuizChapterPdfs(props.lessonId)
...
async function saveQuestion(q: QuizQuestion) {
  try {
    await quizService.teacherUpdateQuestion(q.id, {
      question: q.question,
      option_a: q.option_a,
      option_b: q.option_b,
      option_c: q.option_c,
...
async function deleteQuestion(id: number) {
  try {
    await quizService.teacherDestroyQuestion(id)
