      ...
      <div class="col-span-12 space-y-6 xl:col-span-7">
        ...
        >
          ...
          <div v-if="!loading" class="flex items-end gap-2 h-40">
            ...
            >
              <span
                class="text-xs text-gray-500 opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap dark:text-gray-400"
              >
              ...
              ></div>
              <span class="text-xs text-gray-400">T{{ item.month }}</span>
            </div>
          </div>
        ...
        >
          ...
          <div v-if="!loading" class="space-y-3">
            <div v-for="(course, i) in topCourses" :key="course.id" class="flex items-center gap-3">
              <span
                class="w-7 h-7 rounded-full bg-blue-50 dark:bg-blue-500/10 text-blue-600 dark:text-blue-400 text-xs font-bold flex items-center justify-center shrink-0"
              >
                {{ i + 1 }}
              </span>
              <div class="min-w-0 flex-1">
                ...
                <p class="text-xs text-gray-400">{{ course.sales_count }} lượt bán</p>
              </div>
              <span class="text-sm font-semibold text-gray-700 shrink-0 dark:text-gray-300">{{
                formatCurrency(course.revenue)
              }}</span>
            </div>
            <p v-if="topCourses.length === 0" class="text-sm text-gray-400 text-center py-4">
              Chưa có dữ liệu
            </p>
          </div>
          <div v-else class="space-y-3">
            <div v-for="i in 5" :key="i" class="flex items-center gap-3">
              <div
                class="w-7 h-7 rounded-full bg-gray-100 dark:bg-gray-800 animate-pulse shrink-0"
              ></div>
            ...
            <table class="w-full text-sm">
              <thead>
                <tr class="border-b border-gray-100 dark:border-gray-700">
                  <th
                    class="text-left text-xs font-medium text-gray-500 dark:text-gray-400 pb-3 pr-4"
                  >
                    Học viên
                  </th>
                  <th
                  ...
                  <th class="text-left text-xs font-medium text-gray-500 dark:text-gray-400 pb-3">
                    Trạng thái
                  </th>
                </tr>
              </thead>
              <tbody class="divide-y divide-gray-50 dark:divide-gray-800">
                ...
                >
                  ...
                  <td class="py-3 pr-4 text-gray-600 dark:text-gray-400 max-w-[180px] truncate">
                    {{ order.course_title }}
                  </td>
...
const revenueData = ref<DashboardStats['monthly_revenue']>(
  MONTHS.map((month, i) => ({ month: i + 1, revenue: 0 })),
)
const topCourses = ref<DashboardStats['top_courses']>([])
const recentOrders = ref<DashboardStats['recent_orders']>([])
...
onMounted(async () => {
  try {
    ...
    statValues.value = {
      ...
      courses: data.summary.total_courses,
      orders: data.summary.total_orders,
      revenue: data.summary.total_revenue,
    }

    revenueData.value = data.monthly_revenue
