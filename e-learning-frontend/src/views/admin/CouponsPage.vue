  ...
  <div class="p-6">
    ...
    <div class="flex flex-col sm:flex-row items-start sm:items-center gap-3 mb-4">
      <div class="flex gap-1 p-1 bg-gray-100 rounded-lg">
        <button @click="switchTab(false)" :class="!isTrashed ? 'bg-white shadow-sm font-medium' : ''"
          class="px-3 py-1.5 text-sm rounded-md text-gray-600 transition-colors">Tất cả</button>
        <button @click="switchTab(true)" :class="isTrashed ? 'bg-white shadow-sm font-medium' : ''"
      ...
      <div class="relative flex-1 max-w-xs">
        <input v-model="search" @input="debouncedFetch" type="text" placeholder="Tìm theo mã..."
        ...
        <table class="w-full text-sm">
          <thead>
            <tr class="border-b border-gray-100 bg-gray-50/50">
              <th class="w-10 px-4 py-3"><input type="checkbox" :checked="isAllSelected" :indeterminate="isIndeterminate" @change="toggleSelectAll" class="rounded border-gray-300 text-blue-500" /></th>
          ...
          <tbody class="divide-y divide-gray-50">
            <template v-if="loading">
              <tr v-for="i in 5" :key="i">
                <td class="px-4 py-3"><div class="w-4 h-4 bg-gray-100 rounded animate-pulse"></div></td>
                ...
                <td class="px-4 py-3"><div class="h-4 w-16 bg-gray-100 rounded animate-pulse"></div></td>
                <td class="px-4 py-3"><div class="h-4 w-16 bg-gray-100 rounded animate-pulse"></div></td>
                <td class="px-4 py-3"><div class="h-4 w-28 bg-gray-100 rounded animate-pulse"></div></td>
                <td class="px-4 py-3"><div class="h-4 w-16 bg-gray-100 rounded animate-pulse ml-auto"></div></td>
              </tr>
            </template>
            <template v-else-if="coupons.length">
              <tr v-for="c in coupons" :key="c.id" class="hover:bg-gray-50/50 transition-colors">
                <td class="px-4 py-3"><input type="checkbox" :checked="selectedIds.has(c.id)" @change="toggleSelect(c.id)" class="rounded border-gray-300 text-blue-500" /></td>
                <td class="px-4 py-3"><span class="font-mono font-semibold text-blue-600 text-xs bg-blue-50 px-2 py-1 rounded">{{ c.code }}</span></td>
                <td class="px-4 py-3 text-gray-700 text-xs">
                  <span v-if="c.type === 'fixed'">{{ formatCurrency(Number(c.value)) }}</span>
                  <span v-else>{{ c.value }}%<span v-if="c.max_discount" class="text-gray-400"> (max {{ formatCurrency(Number(c.max_discount)) }})</span></span>
                </td>
                <td class="px-4 py-3 text-xs text-gray-600">{{ c.used_count }}/{{ c.usage_limit ?? '∞' }}</td>
                <td class="px-4 py-3">
                  <button v-if="!isTrashed" @click="doToggleStatus(c)" :disabled="togglingId === c.id"
                  ...
                  <span v-else class="text-xs text-gray-400">{{ c.status === 1 ? 'Hoạt động' : 'Vô hiệu' }}</span>
                </td>
                <td class="px-4 py-3 text-xs text-gray-500">
                  <template v-if="c.start_date || c.end_date">
                    <span v-if="c.is_expired" class="text-red-500 font-medium">Hết hạn</span>
                    <span v-else>{{ c.start_date ? formatDate(c.start_date) : '...' }} — {{ c.end_date ? formatDate(c.end_date) : '...' }}</span>
                  </template>
                  <span v-else class="text-gray-400">Không giới hạn</span>
                </td>
                <td class="px-4 py-3 text-right">
                  <div v-if="!isTrashed" class="flex items-center justify-end gap-1">
                    <button @click="openEdit(c)" class="p-1.5 rounded-lg hover:bg-gray-100 text-gray-500 transition-colors" title="Sửa">
                    ...
                    <button @click="confirmDelete(c)" class="p-1.5 rounded-lg hover:bg-red-50 text-gray-500 hover:text-red-500 transition-colors" title="Xoá">
                  ...
                  <div v-else class="flex items-center justify-end gap-1">
                    <button @click="doRestore(c.id)" :disabled="restoringId === c.id" class="p-1.5 rounded-lg hover:bg-green-50 text-gray-500 hover:text-green-600 transition-colors disabled:opacity-50" title="Khôi phục">
                    ...
                    <button @click="confirmDelete(c)" class="p-1.5 rounded-lg hover:bg-red-50 text-gray-500 hover:text-red-500 transition-colors" title="Xoá vĩnh viễn">
                      <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg>
                    </button>
                  </div>
                </td>
            ...
            <tr v-else><td colspan="7" class="px-4 py-12 text-center text-gray-400 text-sm">{{ isTrashed ? 'Thùng rác trống.' : 'Chưa có mã giảm giá nào.' }}</td></tr>
          </tbody>
        </table>
      </div>
      <div v-if="pagination.last_page > 1" class="flex justify-end px-4 py-3 border-t border-gray-100">
        <PaginationBar :current-page="pagination.current_page" :last-page="pagination.last_page" @change="loadPage" />
      </div>
    </div>

    <!-- Bulk Actions -->
    <div v-if="selectedIds.size > 0" class="fixed bottom-6 left-1/2 -translate-x-1/2 z-[999999] flex items-center gap-3 px-5 py-3 bg-gray-900 text-white rounded-xl shadow-xl">
      <span class="text-sm">Đã chọn <strong>{{ selectedIds.size }}</strong> mã</span>
      <button v-if="!isTrashed" @click="doBulkDelete" :disabled="bulkLoading" class="px-3 py-1.5 text-sm bg-red-500 hover:bg-red-600 rounded-lg disabled:opacity-50">Xoá</button>
      <button v-if="isTrashed" @click="doBulkRestore" :disabled="bulkLoading" class="px-3 py-1.5 text-sm bg-green-500 hover:bg-green-600 rounded-lg disabled:opacity-50">Khôi phục</button>
      <button @click="selectedIds.clear()" class="px-3 py-1.5 text-sm bg-gray-700 hover:bg-gray-600 rounded-lg">Bỏ chọn</button>
    </div>
    ...
    <div v-if="showModal" class="fixed inset-0 z-[999999] flex items-center justify-center bg-black/50" @click.self="showModal = false">
      <div class="bg-white rounded-2xl shadow-xl w-full max-w-lg mx-4 p-6 max-h-[90vh] overflow-y-auto">
        <h3 class="text-lg font-semibold text-gray-800 mb-4">{{ editingCoupon ? 'Sửa mã giảm giá' : 'Thêm mã giảm giá' }}</h3>
        <form @submit.prevent="submitForm" class="space-y-4">
          <div class="grid grid-cols-2 gap-4">
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">Mã giảm giá *</label>
              <input v-model="form.code" type="text" required placeholder="VD: SALE50K" class="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg uppercase font-mono focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none" />
            </div>
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">Loại *</label>
              <select v-model="form.type" class="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500/20 outline-none">
          ...
          <div class="grid grid-cols-2 gap-4">
            <div>
              ...
              <input v-model="form.value" type="number" required min="0" step="any" :placeholder="form.type === 'fixed' ? '50000' : '10'" class="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500/20 outline-none" />
            </div>
            <div v-if="form.type === 'percentage'">
              <label class="block text-sm font-medium text-gray-700 mb-1">Giảm tối đa (VNĐ)</label>
              <input v-model="form.max_discount" type="number" min="0" placeholder="100000" class="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500/20 outline-none" />
            </div>
          </div>
          <div class="grid grid-cols-2 gap-4">
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">Đơn tối thiểu (VNĐ)</label>
              <input v-model="form.min_order_value" type="number" min="0" placeholder="0" class="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500/20 outline-none" />
            </div>
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">Giới hạn sử dụng</label>
              <input v-model="form.usage_limit" type="number" min="1" placeholder="Không giới hạn" class="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500/20 outline-none" />
            </div>
          </div>
          <div class="grid grid-cols-2 gap-4">
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">Ngày bắt đầu</label>
              <input v-model="form.start_date" type="datetime-local" class="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500/20 outline-none" />
            </div>
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">Ngày kết thúc</label>
              <input v-model="form.end_date" type="datetime-local" class="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500/20 outline-none" />
            </div>
          </div>
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Mô tả</label>
            <textarea v-model="form.description" rows="2" placeholder="Mô tả ngắn..." class="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500/20 outline-none resize-none"></textarea>
          </div>
          <p v-if="formError" class="text-sm text-red-500">{{ formError }}</p>
          <div class="flex justify-end gap-2 pt-2">
            <button type="button" @click="showModal = false" class="px-4 py-2 text-sm rounded-lg border border-gray-200 text-gray-600 hover:bg-gray-50">Huỷ</button>
            <button type="submit" :disabled="submitting" class="px-4 py-2 text-sm rounded-lg bg-blue-500 text-white hover:bg-blue-600 disabled:opacity-50">{{ submitting ? 'Đang lưu...' : 'Lưu' }}</button>
          </div>
        </form>
      </div>
    </div>
    ...
    <ConfirmModal :show="showDeleteModal" :title="isTrashed ? 'Xoá vĩnh viễn' : 'Xác nhận xoá'" :loading="deleteLoading"
      ...
      <p>Bạn có chắc muốn xoá mã <strong class="text-gray-800 font-mono">{{ deletingCoupon?.code }}</strong>?
        <span v-if="!isTrashed" class="block mt-1 text-xs text-gray-400">Mã sẽ được chuyển vào thùng rác.</span>
...
const isAllSelected = computed(() => coupons.value.length > 0 && selectedIds.value.size === coupons.value.length)
...
const formError = ref('')
const form = reactive({
  code: '', type: 'fixed' as 'fixed' | 'percentage', value: '',
  min_order_value: '', max_discount: '', usage_limit: '',
  start_date: '', end_date: '', description: ''
})

function resetForm() {
  Object.assign(form, { code: '', type: 'fixed', value: '', min_order_value: '', max_discount: '', usage_limit: '', start_date: '', end_date: '', description: '' })
}
...
async function submitForm() {
  ...
  formError.value = ''
  try {
    const data: Record<string, unknown> = { code: form.code, type: form.type, value: Number(form.value) }
    if (form.min_order_value) data.min_order_value = Number(form.min_order_value)
    if (form.max_discount) data.max_discount = Number(form.max_discount)
    if (form.usage_limit) data.usage_limit = Number(form.usage_limit)
...
function confirmDelete(c: Coupon) { deletingCoupon.value = c; showDeleteModal.value = true }
