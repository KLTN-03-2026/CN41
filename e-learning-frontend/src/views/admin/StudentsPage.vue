<template>
  <div class="p-6">
          ...
          <tbody class="divide-y divide-gray-50 dark:divide-gray-800">
            ...
            <template v-if="loading">
              <tr v-for="i in 5" :key="i">
                ...
                <td class="px-4 py-3">
                  <div
                    class="h-4 w-16 bg-gray-100 dark:bg-gray-800 rounded animate-pulse ml-auto"
                  ></div>
                </td>
              </tr>
            </template>
              ...
              >
                ...
                <td class="px-4 py-3">
                  <div class="flex items-center gap-3">
                    <div
                      class="w-8 h-8 rounded-full bg-blue-100 dark:bg-blue-500/10 flex items-center justify-center text-blue-600 dark:text-blue-400 font-medium text-xs shrink-0"
                    >
                ...
                <td class="px-4 py-3 text-right">
                  <div v-if="!isTrashed" class="flex items-center justify-end gap-1">
                    <button
                      @click="openDetail(s)"
                      class="p-1.5 rounded-lg hover:bg-blue-50 dark:hover:bg-blue-500/10 text-gray-500 dark:text-gray-400 hover:text-blue-500 transition-colors"
                      title="Xem chi tiết"
                    >
                      <svg
                        class="w-4 h-4"
    ...
    >
      <span class="text-sm"
        >Đã chọn <strong>{{ selectedIds.size }}</strong> học viên</span
      >
      <button
        v-if="!isTrashed"
        @click="doBulkDelete"
        :disabled="bulkLoading"
        class="px-3 py-1.5 text-sm bg-red-500 hover:bg-red-600 rounded-lg transition-colors disabled:opacity-50"
      >
        Xoá
      </button>
      <button
        ...
        @click="doBulkRestore"
        :disabled="bulkLoading"
        class="px-3 py-1.5 text-sm bg-green-500 hover:bg-green-600 rounded-lg transition-colors disabled:opacity-50"
      >
        Khôi phục
      </button>
      <button
        ...
        <form @submit.prevent="submitForm" class="space-y-4">
          ...
          <div class="flex justify-end gap-2 pt-2">
            <button
              ...
              class="px-4 py-2 text-sm rounded-lg border border-gray-200 dark:border-gray-700 text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-white/5 transition-colors"
            >
              Huỷ
            </button>
            <button
    ...
    <div
      v-if="showDetail"
      class="fixed inset-0 z-[999999] flex items-center justify-center bg-black/70 backdrop-blur-sm"
      @click.self="showDetail = false"
    >
      <div
        class="bg-white dark:bg-gray-900 rounded-2xl shadow-xl w-full max-w-lg mx-4 max-h-[85vh] flex flex-col"
      >
        <div
        ...
        >
          ...
          <button
            ...
            class="p-1 rounded-lg hover:bg-gray-100 dark:hover:bg-white/10 text-gray-400 transition-colors"
          >
            <svg
              class="w-5 h-5"
              fill="none"
        ...
        <div v-else-if="detailStudent" class="overflow-y-auto p-6 space-y-5">
          ...
          <div class="flex items-center gap-4">
            <div
              class="w-14 h-14 rounded-full bg-blue-100 dark:bg-blue-500/10 flex items-center justify-center text-blue-600 dark:text-blue-400 font-bold text-xl shrink-0"
            >
          ...
          <div class="grid grid-cols-2 gap-3 text-sm">
            <div class="bg-gray-50 dark:bg-white/5 rounded-xl p-3">
              ...
              <p class="font-medium text-gray-700 dark:text-gray-200">
                {{ detailStudent.date_of_birth ? formatDate(detailStudent.date_of_birth) : '—' }}
              </p>
            </div>
            <div class="bg-gray-50 dark:bg-white/5 rounded-xl p-3">
              ...
              <p
                class="font-medium"
                :class="
                  detailStudent.email_verified_at
                    ? 'text-green-600 dark:text-green-400'
                    : 'text-gray-400'
                "
              >
                {{ detailStudent.email_verified_at ? 'Đã xác minh' : 'Chưa' }}
              </p>
            </div>
            <div class="bg-gray-50 dark:bg-white/5 rounded-xl p-3">
              <p class="text-gray-400 text-xs">Đơn hàng</p>
              <p class="font-medium text-gray-700 dark:text-gray-200">
                {{ detailStudent.orders_count ?? 0 }}
              </p>
            </div>
            <div class="bg-gray-50 dark:bg-white/5 rounded-xl p-3">
              <p class="text-gray-400 text-xs">Tổng chi tiêu</p>
              <p class="font-medium text-gray-700 dark:text-gray-200">
                {{ formatCurrency(detailStudent.total_spent ?? 0) }}
              </p>
            </div>
          </div>
          <!-- Enrolled Courses -->
          <div>
            <h4 class="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">
              Khóa học đã đăng ký ({{ detailStudent.enrolled_courses?.length ?? 0 }})
            </h4>
            <div
              v-if="detailStudent.enrolled_courses?.length"
              class="space-y-2 max-h-48 overflow-y-auto"
            >
              <div
                v-for="c in detailStudent.enrolled_courses"
                :key="c.id"
              ...
              >
                <img
                  v-if="c.thumbnail"
                  :src="getThumbnail(c.thumbnail)"
                  class="w-12 h-8 rounded-lg object-cover shrink-0"
                />
                ...
                <div class="flex-1 min-w-0">
                  ...
                  <p class="text-xs text-gray-400">
                    {{ formatCurrency(c.sale_price ?? c.price) }} ·
                    {{ c.enrolled_at ? formatDate(c.enrolled_at) : '' }}
                  </p>
                </div>
              </div>
            </div>
    ...
    >
      <p>
        Bạn có chắc muốn xoá học viên
        <strong class="text-gray-800 dark:text-white/90">{{ deletingStudent?.name }}</strong
        >?
        <span v-if="!isTrashed" class="block mt-1 text-xs text-gray-400"
          >Học viên sẽ được chuyển vào thùng rác.</span
        >
...
const isAllSelected = computed(
  () => students.value.length > 0 && selectedIds.value.size === students.value.length,
)
...
function confirmForceDelete(s: Student) {
  deletingStudent.value = s
  showDeleteModal.value = true
}
