<template>
  <div class="p-6">
              ...
              >
                ...
                <td class="px-4 py-3">
                  <div class="flex items-center gap-3">
                    ...
                    <div
                      v-else
                      class="w-8 h-8 rounded-full bg-purple-100 dark:bg-purple-500/10 flex items-center justify-center text-purple-600 dark:text-purple-400 font-medium text-xs shrink-0"
                    >
    ...
    >
      <span class="text-sm"
        >Đã chọn <strong>{{ selectedIds.size }}</strong> giảng viên</span
      >
      <button
        v-if="!isTrashed"
        @click="doBulkDelete"
        ...
        <form @submit.prevent="submitForm" class="space-y-4">
          ...
          <div>
            ...
            <textarea
              ...
              rows="3"
              class="w-full px-3 py-2 text-sm border border-gray-200 dark:border-gray-700 rounded-lg bg-white dark:bg-white/5 text-gray-700 dark:text-gray-300 focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none resize-none"
            ></textarea>
          </div>
          <div>
            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1"
    ...
    >
      <p>
        ...
        <strong class="text-gray-800 dark:text-white/90">{{ deletingTeacher?.name }}</strong
        >?
        <span v-if="!isTrashed" class="block mt-1 text-xs text-gray-400"
          >Giảng viên sẽ được chuyển vào thùng rác.</span
        >
...
const isAllSelected = computed(
  () => teachers.value.length > 0 && selectedIds.value.size === teachers.value.length,
)
...
function openEdit(t: Teacher) {
  ...
  Object.assign(form, { name: t.name, description: t.description || '', exp: t.exp || '' })
  formError.value = ''
  showModal.value = true
}
