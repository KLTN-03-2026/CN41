<template>
  <div
    class="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-50 to-blue-50 px-4 py-12"
  >
    <div class="w-full max-w-md">
      ...
      <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-8 text-center">
        ...
        <template v-if="status === 'success'">
          ...
          <p class="text-gray-500 mb-8">
            Email của bạn đã được xác thực. Tài khoản đã được kích hoạt, bạn có thể bắt đầu học
            ngay.
          </p>
          <button
            @click="goToLogin"
            class="btn-primary w-full flex justify-center items-center py-3 rounded-xl font-semibold"
          >
            Đăng nhập ngay
          </button>
        </template>
        ...
        <template v-else-if="status === 'expired'">
          ...
          <p class="text-gray-500 mb-8">
            Link xác thực chỉ có hiệu lực trong <strong>24 giờ</strong> và đã hết hạn. Vui lòng đăng
            nhập để yêu cầu gửi lại link mới.
          </p>
          <router-link
            to="/login"
            class="btn-primary w-full flex justify-center items-center py-3 rounded-xl font-semibold"
        ...
        <template v-else>
          ...
          <p class="text-gray-500 mb-8">
            Link xác thực không hợp lệ hoặc đã được sử dụng. Vui lòng đăng nhập để yêu cầu gửi lại
            link mới.
          </p>
          <router-link
            to="/login"
      ...
      <p class="text-center text-sm text-gray-400 mt-6">
        Cần hỗ trợ?
        <a href="mailto:support@elearning.com" class="text-primary-600 hover:underline"
          >Liên hệ chúng tôi</a
        >
      </p>
    </div>
...
import { CheckCircle2, ShieldCheck, Clock, XCircle } from 'lucide-vue-next'
