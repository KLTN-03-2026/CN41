...
class UploadController extends Controller
{
    ...
    public function hlsKey(int $id, Request $request)
    {
        $this->authorizeStreamRequest($request);

        $mediaFile = $this->uploadService->findOrFail($id);

        if (! $mediaFile->hls_key) {
            abort(404, 'Key không tồn tại.');
        }

        return response(hex2bin($mediaFile->hls_key), 200, [
            'Content-Type' => 'application/octet-stream',
            'Cache-Control' => 'no-cache, no-store, must-revalidate',
        ]);
    }

    public function streamDocument(int $id, Request $request): Response
    {
        $this->authorizeStreamRequest($request);
        ...
        $pdfBytes = Storage::disk($mediaFile->disk)->get($mediaFile->path);
        ...
        $email = '';
        if ($student = Auth::guard('api')->user()) {
            $email = $student->email;
